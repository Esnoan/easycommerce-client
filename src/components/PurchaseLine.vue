<template>
  <div class="form-group row">
    ...
    <div class="col-sm-2">
      <input type="number" :class="classA" v-bind:value="valueAmount" v-on="inputCListeners">
    </div>
    <div class="col-sm-1">
...
export default {
  props: ['id', 'valueProduct', 'valueQuantity', 'valueAmount', 'errorP', 'errorQ', 'errorA'],
  mounted() {
    if (this.isLoggedIn) {
      this.index();
    }
  ...
  computed: {
    ...
    classA() {
      return this.errorA ? 'form-control fill form-control-danger ' : 'form-control fill';
    },
    inputAListeners() {
      const vm = this;
    ...
    inputCListeners() {
      ...
      return Object.assign(
        ...
        // behavior of some listeners.
        {
          // This ensures that the component works with v-model
          input(event) {
            vm.$emit('inputC', `${event.target.value}:${x}`);
          },
        },
      );
    },
